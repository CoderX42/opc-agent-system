...
export class FinanceService {
  ...
  async createInvoice(
    ...
    const invoice = this.invoiceRepository.create({
      ...
      invoiceNo:
        dto.invoiceNo || `INV-${randomUUID().slice(0, 8).toUpperCase()}`,
  ...
  async updateInvoice(ownerId: string, id: string, dto: UpdateInvoiceDto) {
    ...
    if (dto.amount !== undefined || dto.taxRate !== undefined) {
      invoice.taxAmount =
        Math.round(
          Number(invoice.amount) * (Number(invoice.taxRate) / 100) * 100,
        ) / 100;
    }
    const saved = await this.invoiceRepository.save(invoice);
    await this.invalidate(ownerId);
  ...
  async getFinancialSummary(ownerId: string) {
    ...
    const [transactionTotals, transactionCount, invoiceTotals] =
      await Promise.all([
    ...
    const totalIncome =
      Number(transactionTotals?.income || 0) +
      Number(invoiceTotals?.income || 0);
    const totalExpense =
      Number(transactionTotals?.expense || 0) +
      Number(invoiceTotals?.expense || 0);
    const summary = {
      totalIncome,
      totalExpense,
  ...
  async getOverview(ownerId: string) {
    const [
      ...
      this.invoiceRepository
        .createQueryBuilder("i")
        .select(`DATE_FORMAT(i.date, '%Y-%m')`, "month")
        .addSelect(
        ...
        .andWhere("i.status = :status", { status: "APPROVED" })
        .setParameters({ income: "income", expense: "expense" })
        .groupBy(`DATE_FORMAT(i.date, '%Y-%m')`)
        .orderBy("month", "ASC")
        .limit(12)
        .getRawMany(),
      this.transactionRepository
  ...
  private mergeCategoryDistribution(
    ...
    for (const group of groups) {
      for (const item of group) {
        ...
        byCategory.set(
          category,
          (byCategory.get(category) || 0) + Number(item.amount || 0),
        );
      }
    }
  ...
  private parseInvoiceOcrText(text: string) {
    ...
    return {
      ...
      description: normalized
        ? `OCR识别文本：${normalized.slice(0, 500)}`
        : undefined,
    };
  }
  ...
  > {
    ...
    const cacheVersion =
      (await this.cache.get<number>(`finance:kline:version:${ownerId}`)) || 1;
    ...
    const series = this.periodStarts(windowStart, today, safeDimension).map(
      (period) => {
