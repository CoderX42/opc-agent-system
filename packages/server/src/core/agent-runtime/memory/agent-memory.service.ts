...
export class AgentMemoryService {
  ...
  async getShortTerm(userId: string, sessionId?: string): Promise<string[]> {
    if (!sessionId) return [];
    try {
      return (await this.cache.get<string[]>(this.shortTermKey(userId, sessionId))) || [];
    } catch (error) {
  ...
  async upsertLongTerm(input: {
    ...
    const existing = await this.memories
      ...
      .andWhere(input.agentId ? 'memory.agentId = :agentId' : 'memory.agentId IS NULL', {
        agentId: input.agentId,
      })
      .andWhere('memory.memoryType = :memoryType', { memoryType: input.memoryType })
  ...
  async findLongTerm(
    ...
    if (query) {
      const keywords = query.split(/[\s,，。.、]+/).filter((item) => item.length > 1).slice(0, 5);
      if (keywords.length > 0) {
        builder.andWhere(
          keywords
            .map((_, index) => `(memory.key ILIKE :kw${index} OR memory.content ILIKE :kw${index})`)
            .join(' OR '),
          Object.fromEntries(keywords.map((keyword, index) => [`kw${index}`, `%${keyword}%`])),
        );
      }
    }
